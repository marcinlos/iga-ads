...
public:
    ...
    auto u_with_grad(point p) const noexcept -> ads::value_type {
        auto const [x, y] = p;
        auto const val = y + 0.5 * x <= 0.5 ? 1.0 : 0.0;
        return {val, 0.0, 0.0};
    }
...
auto adapted_points(double a, double b, int elems, double d) -> ads::partition {
    auto const n = elems / 2;
    auto const left = ads::evenly_spaced(0.0, d, n);
    auto const right = ads::evenly_spaced(d, 1.0, n);
...
auto galerkin(int argc, char* argv[]) -> void {
    ...
    auto const beta = ads::point_t{std::sqrt(3), 1};
    auto const problem = skew_to_mesh{eps};

    // auto const xs = ads::evenly_spaced(0.0, 1.0, elems_x);
    // auto const ys = ads::evenly_spaced(0.0, 1.0, elems_y);
    auto const d = 0.1;
    auto const xs = adapted_points(0.0, 1.0, elems_x, d);
    auto const ys = adapted_points(0.0, 1.0, elems_y, d);
    ...
    if (!args.weak_bc) {
        ...
        auto bufy = ads::lin::vector{{dimy.dofs()}};
        ads::compute_projection(bufy, dimy.basis, [problem](auto t) { return problem.u({0, t}); });
        ads::lin::solve_with_factorized(dimy.M, bufy, dimy.ctx);
        for (int i = 0; i < dimy.dofs(); ++i) {
            F[U.global_index({0, i})] = bufy(i);
        }
        auto dimx = ads::dimension{bx, p + 1, 1};
        dimx.factorize_matrix();
        auto bufx = ads::lin::vector{{dimx.dofs()}};
        ads::compute_projection(bufx, dimx.basis, [problem](auto t) { return problem.u({t, 0}); });
        ads::lin::solve_with_factorized(dimx.M, bufx, dimx.ctx);
        for (int i = 0; i < dimx.dofs(); ++i) {
            F[U.global_index({i, 0})] = bufx(i);
        }
    } else {
        // Weak BC
        auto const d = 2;
...
auto igrm(int argc, char* argv[]) -> void {
    ...
    assemble(V, quad, G, [=](auto u, auto v, auto /*x*/) {
        return u.val * v.val + eps * dot(grad(u), grad(v));
    });
    ...
    if (args.weak_bc) {
        ...
        fmt::print("Strong BC not supported\n");
        std::exit(1);
    }
    ...
    solver.solve(linear_system, "skew-matrix");
...
auto save_mesh(ads::partition const& xs, ads::partition const& ys, int iter) -> void {
    auto out_x = fmt::output_file(fmt::format("xs-{}", iter));
    for (auto const& x : xs) {
        out_x.print("{}\n", x);
    }

    auto out_y = fmt::output_file(fmt::format("ys-{}", iter));
    for (auto const& y : ys) {
        out_y.print("{}\n", y);
    }
}
