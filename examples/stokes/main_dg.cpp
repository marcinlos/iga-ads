...
int main(int argc, char* argv[]) {
    ...
    using std::max;
    int pmax_trial_x = max(max(vxp_trial_x, vyp_trial_x), pp_trial_x);
    int pmax_trial_y = max(max(vxp_trial_y, vyp_trial_y), pp_trial_y);
    int pmax_test_x = max(max(vxp_test_x, vyp_test_x), pp_test_x);
    int pmax_test_y = max(max(vxp_test_y, vyp_test_y), pp_test_y);

    int p_max = max(max(pmax_trial_x, pmax_trial_y), max(pmax_test_x, pmax_test_y));
    ...
    // Pressure spaces
    auto trial_basis_x = ads::bspline::create_basis(0, 1, pp_trial_x, n, p_rep_trial_x);
    auto dtrial_x = ads::dimension{trial_basis_x, quad, ders, subdivision};

    auto trial_basis_y = ads::bspline::create_basis(0, 1, pp_trial_y, n, p_rep_trial_y);
    auto dtrial_y = ads::dimension{trial_basis_y, quad, ders, subdivision};

    auto test_basis_x = ads::bspline::create_basis(0, 1, pp_test_x, subdivision * n, p_rep_test_x);
    auto dtest_x = ads::dimension{test_basis_x, quad, ders, 1};

    auto test_basis_y = ads::bspline::create_basis(0, 1, pp_test_y, subdivision * n, p_rep_test_y);
    auto dtest_y = ads::dimension{test_basis_y, quad, ders, 1};

    // Velocity spaces
    using ads::bspline::create_basis;
    auto U1_trial_basis_x = create_basis(0, 1, vxp_trial_x, n, vx_rep_trial_x);
    auto U1_dtrial_x = ads::dimension{U1_trial_basis_x, quad, ders, subdivision};

    auto U1_trial_basis_y = create_basis(0, 1, vxp_trial_y, n, vx_rep_trial_y);
    auto U1_dtrial_y = ads::dimension{U1_trial_basis_y, quad, ders, subdivision};

    auto U1_test_basis_x = create_basis(0, 1, vxp_test_x, subdivision * n, vx_rep_test_x);
    auto U1_dtest_x = ads::dimension{U1_test_basis_x, quad, ders, 1};

    auto U1_test_basis_y = create_basis(0, 1, vxp_test_y, subdivision * n, vx_rep_test_y);
    auto U1_dtest_y = ads::dimension{U1_test_basis_y, quad, ders, 1};

    auto U2_trial_basis_x = create_basis(0, 1, vyp_trial_x, n, vy_rep_trial_x);
    auto U2_dtrial_x = ads::dimension{U2_trial_basis_x, quad, ders, subdivision};

    auto U2_trial_basis_y = create_basis(0, 1, vyp_trial_y, n, vy_rep_trial_y);
    auto U2_dtrial_y = ads::dimension{U2_trial_basis_y, quad, ders, subdivision};

    auto U2_test_basis_x = create_basis(0, 1, vyp_test_x, subdivision * n, vy_rep_test_x);
    auto U2_dtest_x = ads::dimension{U2_test_basis_x, quad, ders, 1};

    auto U2_test_basis_y = create_basis(0, 1, vyp_test_y, subdivision * n, vy_rep_test_y);
    auto U2_dtest_y = ads::dimension{U2_test_basis_y, quad, ders, 1};

    // Sanity check
    auto trial_v_dim =
        U1_dtrial_x.dofs() * U1_dtrial_y.dofs() + U2_dtrial_x.dofs() * U2_dtrial_y.dofs();
    ...
    if (trial_dim > test_dim) {
        ...
        std::cerr << "dim P =  " << dtrial_x.dofs() * dtrial_y.dofs() << " = " << dtrial_x.dofs()
                  << " x " << dtrial_y.dofs() << std::endl;
        ...
        std::cerr << "dim Q =  " << dtest_x.dofs() * dtest_y.dofs() << " = " << dtest_x.dofs()
                  << " x " << dtest_y.dofs() << std::endl;
        std::exit(1);
    } else {
    ...
    auto sim = ads::stokes_dg{trial, test, steps};
    sim.run();
}
