...
#ifndef STOKES_STOKES_CONFORMING_HPP
#define STOKES_STOKES_CONFORMING_HPP
...
class stokes_conforming : public simulation_2d {
private:
...
public:
    stokes_conforming(space_set trial_, space_set test_, const timesteps_config& steps)
    ...
    void assemble_matrix(mumps::problem& problem) const {
        ...
        for (auto i : dofs(test.U1x, test.U1y)) {
            for (auto j : dofs(test.U2x, test.U2y)) {
                ...
                int ii = linear_index(i, test.U1x, test.U1y) + 1;
                int jj = linear_index(j, test.U2x, test.U2y) + 1;
                auto eval = [&](auto form) { return integrate(i, j, test.U1x, test.U1y, test.U2x,
        test.U2y, form); };
        ...
        */
        ...
        for (auto i : dofs(test.Px, test.Py)) {
            for (auto j : dofs(trial.U1x, trial.U1y)) {
                ...
                put(ii, jj, DU1 + DU2, 0, eval([](auto w, auto u) { return w.val * u.dx; }),
                    fixed_i, bd_j);
            }
        }
        ...
        for (auto i : dofs(test.Px, test.Py)) {
            for (auto j : dofs(trial.U2x, trial.U2y)) {
                ...
                put(ii, jj, DU1 + DU2, dU1, eval([](auto w, auto u) { return w.val * u.dy; }),
                    fixed_i, bd_j);
            }
        }
        ...
        for (auto i : dofs(test.U1x, test.U1y)) {
            for (auto j : dofs(trial.Px, trial.Py)) {
                ...
                put(ii, jj, 0, dU1 + dU2, eval([](auto w, auto u) { return -w.dx * u.val; }), bd_i,
                    fixed_j);
            }
        }
        ...
        for (auto i : dofs(test.U2x, test.U2y)) {
            for (auto j : dofs(trial.Px, trial.Py)) {
                ...
                put(ii, jj, DU1, dU1 + dU2, eval([](auto w, auto u) { return -w.dy * u.val; }),
                    bd_i, fixed_j);
            }
        }
...
#endif  // STOKES_STOKES_CONFORMING_HPP
