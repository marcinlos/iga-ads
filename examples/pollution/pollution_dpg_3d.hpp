...
private:
    ...
    void compute_rhs_x() {
        ...
        executor.for_each(elements(Vx, Uy, Uz), [&](index_type e) {
            ...
            executor.synchronized([&]() { update_global_rhs(rhs1, U, e, Vx, Uy, Uz); });
        });
    }
    ...
    void compute_rhs_y() {
        ...
        executor.for_each(elements(Ux, Vy, Uz), [&](index_type e) {
            ...
            executor.synchronized([&]() { update_global_rhs(rhs2, U, e, Ux, Vy, Uz); });
        });
    }
    ...
    void compute_rhs_z() {
        ...
        executor.for_each(elements(Ux, Uy, Vz), [&](index_type e) {
            ...
            executor.synchronized([&] { update_global_rhs(rhs2, U, e, Ux, Uy, Vz); });
        });
    }
    ...
    void analyze(int iter, double /*t*/) {
        ...
        std::cout << "Step " << (iter + 1) << ": L2 = " << std::sqrt(total) << ", emission = " << em
                  << std::endl;
    }
};
