...
private:
    ...
    void prepare_matrices() {
        ...
        matrix(E1_1, tau, 0, 1, 0,
               [this](auto i) { return is_boundary(i[1], Vy) || is_boundary(i[2], Vz); });
        matrix(E2_1, tau, 0, 0, 1,
               [this](auto i) { return is_boundary(i[0], Vx) || is_boundary(i[2], Vz); });
        matrix(E3_1, tau, 1, 0, 0,
               [this](auto i) { return is_boundary(i[0], Vx) || is_boundary(i[1], Vy); });
    ...
    void step(int /*iter*/, double /*t*/) override {
        ...
        compute_rhs(rhs_E1, prev, prev.E1, prev.E2, prev.E3, UE1x, UE1y, UE1z,
                    [=](auto E, auto, auto H, auto v, auto x) {
                        return (E[X].val + a(x) * (H[Z].dy - H[Y].dz)) * v.val
                             + b(x) * E[Y].dx * v.dy;
                    });
        ...
        compute_rhs(rhs_E2, prev, prev.E1, prev.E2, prev.E3, UE2x, UE2y, UE2z,
                    [=](auto E, auto, auto H, auto v, auto x) {
                        return (E[Y].val + a(x) * (H[X].dz - H[Z].dx)) * v.val
                             + b(x) * E[Z].dy * v.dz;
                    });
        ...
        compute_rhs(rhs_E3, prev, prev.E1, prev.E2, prev.E3, UE3x, UE3y, UE3z,
                    [=](auto E, auto, auto H, auto v, auto x) {
                        return (E[Z].val + a(x) * (H[Y].dx - H[X].dy)) * v.val
                             + b(x) * E[X].dz * v.dx;
                    });
        ...
        compute_rhs(rhs_H1, prev, rhs_E1, rhs_E2, rhs_E3, UH1x, UH1y, UH1z,
                    [=](auto E, auto En, auto H, auto v, auto x) {
    ...
    void after_step(int iter, double t) override {
        ...
        if (i % 10 == 0) {
            output.to_file("out_%d.vti", i, output.evaluate(now.E1), output.evaluate(now.E2),
                           output.evaluate(now.E3), output.evaluate(now.H1),
                           output.evaluate(now.H2), output.evaluate(now.H3));
        }
