...
int main(int argc, char* argv[]) {
    if (argc != 9) {
        std::cerr << "Usage: erikkson_nonstationary <type> <Nx> <Ny> <p_trial> <C_trial> <p_test> <C_test> <steps>" << std::endl;
        std::exit(1);
    }
    ...
    // if (type == "supg-weak") erikkson_supg_weak{dtrial_x, dtrial_y, steps}.run();
    ...
    scheme method;
    if (type == "BE") method = scheme::BE;
    else if (type == "CN") method = scheme::CN;
    else if (type == "PR") method = scheme::peaceman_rachford;
    else if (type == "strang-BE") method = scheme::strang_BE;
    else if (type == "strang-CN") method = scheme::strang_CN;
