...
#ifndef ERIKKSON_ERIKKSON_QUANLING_HPP
#define ERIKKSON_ERIKKSON_QUANLING_HPP
...
public:
    erikkson_quanling(dimension trial_x, dimension trial_y, dimension test_x, dimension test_y,
...
private:
    ...
    void assemble_problem(mumps::problem& problem, const dimension& Vx, const dimension& Vy,
        ...
        for (auto ix = 1; ix < Vx.dofs() - 1; ++ix) {
            for (auto iy = 1; iy < Vy.dofs() - 1; ++iy) {
                ...
                for (auto jx = max(1, ix - Vx.B.degree);
                     jx < min(Vx.dofs() - 1, ix + Vx.B.degree + 1); ++jx) {
                    for (auto jy = max(1, iy - Vy.B.degree);
                         jy < min(Vy.dofs() - 1, iy + Vy.B.degree + 1); ++jy) {
        ...
        //                 double val = 0;
        //                 val += c_diff[0] * M.KUVx(ix, jx) * M.MUVy(iy, jy) + beta[0] * M.AUVx(ix,
        //                 jx) * M.MUVy(iy, jy); val += c_diff[1] * M.MUVx(ix, jx) * M.KUVy(iy, jy)
        //                 + beta[1] * M.MUVx(ix, jx) * M.AUVy(iy, jy); if (val != 0) {
        //                     if (ix != 0 && ix != Vx.dofs() - 1 && iy != 0 && iy != Vy.dofs() - 1
        //                         && jx != 0 && jx != Ux.dofs() - 1 && jy != 0 && jy != Uy.dofs() -
        //                         1 ) { problem.add(i, N + j, val);
        //                     }
        ...
        for (auto ix = 1; ix < Ux.dofs() - 1; ++ix) {
            for (auto iy = 1; iy < Uy.dofs() - 1; ++iy) {
                ...
                for (auto jx = max(1, ix - Ux.B.degree);
                     jx < min(Ux.dofs() - 1, ix + Ux.B.degree + 1); ++jx) {
                    for (auto jy = max(1, iy - Uy.B.degree);
                         jy < min(Uy.dofs() - 1, iy + Uy.B.degree + 1); ++jy) {
                        int j = &u(jx, jy) - &u(0, 0) + 1;
    ...
    void apply_bc(vector_type& u_rhs) {
        ...
        compute_projection(buf_x0, Uy.basis, [&](double t) { return std::sin(M_PI * t); });
    ...
    value_type exact(double x, double y, double eps) const {
        ...
        return {val, (r1 * exp(r1 * (x - 1)) - r2 * exp(r2 * (x - 1))) / norm * std::sin(M_PI * y),
...
#endif  // ERIKKSON_ERIKKSON_QUANLING_HPP
