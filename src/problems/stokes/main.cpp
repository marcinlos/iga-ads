...
int main(int argc, char* argv[]) {
    if (argc != 26) {
        std::cerr << "Usage: stokes <N> <spaces...>" << std::endl;
        std::exit(1);
    }
    int idx = 1;
    ...
    n = 64;
    auto U1_ref_x = make_dimension(vxp_trial_x, vxc_trial_x, n, quad, ders);
    auto U1_ref_y = make_dimension(vxp_trial_y, vxc_trial_y, n, quad, ders);

    auto U2_ref_x = make_dimension(vyp_trial_x, vyc_trial_x, n, quad, ders);
    auto U2_ref_y = make_dimension(vyp_trial_y, vyc_trial_y, n, quad, ders);
