...
struct projector {
    ...
    struct evaluator {
        ...
        void dof_loop(const index_type& e, const index_type& q, index_type& a, double J, double w,
                      const point_type& x, std::size_t level) {
            if (level < N) {
                auto& basis = proj.bases[level];
        ...
        double& rhs(index_type a) { return indexer(a); }

        double call_f(point_type x) { return call(x); }

        template <typename... Idx>
        std::enable_if_t<sizeof...(Idx) == N, double&> indexer(const index_type&, Idx... indices) {
            return u(indices...);
        }

        template <typename... Idx>
        std::enable_if_t<sizeof...(Idx) < N, double&> indexer(const index_type& idx,
                                                              Idx... indices) {
            return indexer(idx, idx[N - sizeof...(Idx) - 1], indices...);
        }

        template <typename... Xs>
        std::enable_if_t<sizeof...(Xs) == N, double> call(const point_type&, Xs... xs) {
            return f(xs...);
        }

        template <typename... Xs>
        std::enable_if_t<sizeof...(Xs) < N, double> call(const point_type& x, Xs... xs) {
...
template <typename Rhs, typename Function>
void compute_projection(Rhs& u, const basis_data& d1, const basis_data& d2, const basis_data& d3,
                        Function&& f) {
